...
class DataCollector(Node):
    def __init__(self, dataset):
        ...
        self.frame_count = 0
        # Buffer obs from previous tick. Paired with NEXT tick's state as action[t]=state[t+1].
        ...
        print("\033[36m [EXP] action[t] = state[t+1] (controller-agnostic next-state label)\033[0m")
    ...
    def on_btn_save_episode(self):
        ...
        with self.lock:
            if self.is_recording and self.frame_count > 0:
                ...
                self.frame_count = 0
                self.pending_obs = None  # discard buffered last obs (no state[t+1] available)
            else:
    ...
    def recording_callback(self):
        with self.lock:
            ...
            # action[t] = state[t+1]:
            #   arm:     realized joint state one frame ahead (current arm_position)
            #   gripper: commanded gripper target at this tick (preserves close-intent,
            #            avoids teaching under-closed grips when finger is blocked)
            if self.pending_obs is not None:
                action = np.append(arm_position, gripper_cmd).astype(np.float32)
                frame = {**self.pending_obs, "action": action}
                with self.dataset_lock:
                    self.dataset.add_frame(frame, task=self.task)
                self.frame_count += 1

            self.pending_obs = current_obs
